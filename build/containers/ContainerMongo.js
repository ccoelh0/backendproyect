"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const mongoose_1 = __importDefault(require("mongoose"));
const config_1 = __importDefault(require("../config"));
const mongoConnection = () => __awaiter(void 0, void 0, void 0, function* () { return yield mongoose_1.default.connect(config_1.default.mongobd.connection); });
mongoConnection();
class Container {
    constructor(collectionName, schema) {
        this.collection = mongoose_1.default.model(collectionName, schema);
    }
    save(object) {
        return __awaiter(this, void 0, void 0, function* () {
            const obj = new this.collection(object);
            const objSaved = yield obj.save();
            return objSaved;
        });
    }
    getAll() {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.collection.find();
        });
    }
    getById(id) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.collection.findOne({ _id: id });
        });
    }
    deleteById(id) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.collection.findOneAndDelete({ _id: id });
        });
    }
    updateById(id, edit) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.collection.findByIdAndUpdate({ _id: id }, edit);
        });
    }
}
exports.default = Container;
