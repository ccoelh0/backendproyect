...
const item = "632bae78614b9cf26c19826a";
const error = "aaa";
...
describe("TEST API/CART:", () => {
  describe("Create new cart", () => {
    ...
    it("Res equals to email is undefined", async () => {
      const res = await request.post(url, {});
      expect(res.body).to.eql({ err: "email is undefined" });
    });
  });
  ...
  describe("Get all carts:", () => {
    ...
    it("Res should be not found", async () => {
      const res = await request.get(url + "1111");
  ...
  describe("Delete cart and items", async () => {
    it("Res should be not found", async () => {
      const res = await request.delete(
        url + "634b16dff4a2a83dba960e7a/" + error
      );
      expect(res.status).to.eql(404);
    });
  ...
  describe("Add items to cart:", () => {
    it("Res equals to 404", async () => {
      const res = await request.post(url + cart + "/items/" + item + "j");
      expect(res.status).to.eql(404);
    });
